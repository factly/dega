...
func claimantUpdateMock(mock sqlmock.Sqlmock, claimant map[string]interface{}, err error) {
	...
	if err != nil {
		...
		claimantSelectWithOutSpace(mock, claimant)
		mock.ExpectCommit()
	}

}

func claimantSelectWithOutSpace(mock sqlmock.Sqlmock, claimant map[string]interface{}) {
...
func claimantSelectWithSpace(mock sqlmock.Sqlmock) {
	mock.ExpectQuery(selectQuery).
		WithArgs(1, 1).
		WillReturnRows(sqlmock.NewRows(columns).
