...
func claimUpdateMock(mock sqlmock.Sqlmock, claim map[string]interface{}, err error) {
	...
	mock.ExpectCommit()
	claimSelectWithOutSpace(mock, updatedClaim)

}

func claimSelectWithOutSpace(mock sqlmock.Sqlmock, claim map[string]interface{}) {
...
func claimSelectWithSpace(mock sqlmock.Sqlmock) {
	mock.ExpectQuery(selectQuery).
		WithArgs(1, 1).
		WillReturnRows(sqlmock.NewRows(columns).
