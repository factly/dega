...
export interface Format {
  id: string;
  [key: string]: any; // Add specific properties based on your format object structure
}
  ...
  return (dispatch: Dispatch) => {
    ...
    return axios
      ...
      .then((response) => {
        ...
        dispatch(
          addFormatsRequest({
            data: response.data.nodes.map((item: Format) => item.id),
            query: query,
...
export const getFormats = (query: any) => {
  return (dispatch: Dispatch) => {
    ...
    return axios
      ...
      .catch((error) => {
        if (error?.response?.status === 307) dispatch(setRedirect(307));
        dispatch(addErrorNotification(getError(error)));
      })
      .finally(() => dispatch(stopFormatsLoading()));
  };
...
export const setRedirect = (code: number) => ({
  type: SET_REDIRECT,
  payload: code,
});
...
export const addFormat = (type: string, payload: any) => ({
  type,
  payload,
});

export const addFormats = (payload: Format[]) => ({
  type: ADD_FORMATS,
  payload,
});

export const addFormatsRequest = (payload: FormatRequest) => ({
