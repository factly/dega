...
var serveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if config.SearchEnabled() {
			...
			err := meilisearchx.SetupMeiliSearch(meiliIndex, []string{"space_id", "name", "slug", "description", "title", "subtitle", "excerpt", "claim", "fact", "meta", "meta_fields"}, []string{"kind", "space_id", "status", "tag_ids", "category_ids", "author_ids", "claimant_id", "rating_id"})
			if err != nil {
				fmt.Println(err)
			}
		}
