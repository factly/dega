...
var reindexCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err = AddMedium(); err != nil {
			log.Fatal(err)
		}

		if err = AddMenu(); err != nil {
			log.Fatal(err)
		}

		if err = AddSpace(); err != nil {
			log.Fatal(err)
		}

		if err = AddClaim(); err != nil {
			log.Fatal(err)
		}

		if err = AddClaimant(); err != nil {
			log.Fatal(err)
		}

		if err = AddRating(); err != nil {
			log.Fatal(err)
		}

		if err = AddPodcast(); err != nil {
			log.Fatal(err)
		}

		if err = AddEpisode(); err != nil {
			log.Fatal(err)
		}
	},
}
...
func AddPosts(page bool) error {
	...
	config.DB.Model(&model.PostAuthor{}).Find(&postAuthors)
	...
	config.DB.Model(&factCheckModel.PostClaim{}).Find(&postClaims)
...
func AddCategories() error {
	...
	config.DB.Model(&model.Category{}).Find(&categories)
...
func AddTags() error {
	...
	config.DB.Model(&model.Tag{}).Find(&tags)
...
func AddMedium() error {
	medium := make([]model.Medium, 0)
	config.DB.Model(&model.Medium{}).Find(&medium)
...
func AddMenu() error {
	...
	config.DB.Model(&model.Menu{}).Find(&menus)
...
func AddSpace() error {
	...
	config.DB.Model(&model.Space{}).Find(&spaces)
...
func AddClaim() error {
	...
	config.DB.Model(&factCheckModel.Claim{}).Find(&claims)
...
func AddClaimant() error {
	claimants := make([]factCheckModel.Claimant, 0)
	config.DB.Model(&factCheckModel.Claimant{}).Find(&claimants)
...
func AddRating() error {
	ratings := make([]factCheckModel.Rating, 0)
	config.DB.Model(&factCheckModel.Rating{}).Find(&ratings)
...
func AddPodcast() error {
	podcasts := make([]podcastModel.Podcast, 0)
	config.DB.Model(&podcastModel.Podcast{}).Preload("Categories").Find(&podcasts)
...
func AddEpisode() error {
	episodes := make([]podcastModel.Episode, 0)
	config.DB.Model(&podcastModel.Episode{}).Find(&episodes)
