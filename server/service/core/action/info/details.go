...
func details(w http.ResponseWriter, r *http.Request) {
	...
	err = config.DB.Raw(fmt.Sprint("SELECT  formats.slug, posts.status, COUNT (*) FROM posts JOIN formats ON posts.format_id = formats.id where posts.deleted_at IS NULL AND posts.is_page = false AND posts.space_id = ", sID, "group by posts.status, formats.slug")).Scan(&result.Posts).Error
