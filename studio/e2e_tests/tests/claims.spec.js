...
test('should create claim successfully...', async ({ page }) => {
  ...
  createdClaimName = claimName; // Store the created claim name
...
test('Should find search results', async ({ page }) => {
  const claimToSearch = createdClaimName;
...
test('should edit a claim successfully...', async ({ page }) => {
  // Generate a random string
  const randomString = getRandomString(10); // Adjust the length as needed
  const claimSelector = `text=${createdClaimName}`;
...
test('should delete a claim successfully', async ({ page }) => {
  const claimText = editedClaimName;
...
test('should display "Please input the Claim!" successfully, when the respective input fields are empty', async ({
  page,
}) => {
...
test('should display "Please add claimant!" successfully, when the claimant input fields are empty', async ({
  page,
}) => {
  test.setTimeout(90000);
  // Click on the 'Create' button
...
test('should display "Please add rating!" successfully, when the claima input field is empty', async ({
  page,
}) => {
  test.setTimeout(90000);
  // Click on the 'Create' button
