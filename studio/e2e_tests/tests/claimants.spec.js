...
let createdClaimantName = ''; // Define a global variable to store the created claimant name
let editedClaimantName = ''; // Define a global variable to store the edited claimant name
...
test('should create claimant successfully', async ({ page }) => {
  ...
  createdClaimantName = `This is a test claimant ${randomString}`;
  await page.type('#creat-claimant_name', createdClaimantName);
  await page.type('#creat-claimant_tag_line', 'New claimant Line'); // Click on the 'Save' button
...
test('should edit a claimant successfully', async ({ page }) => {
  // Generate a random string
  const randomString = getRandomString(10); // Adjust the length as needed
  editedClaimantName = `This is an edited test claimant ${randomString}`;
  const claimantSelector = `text=${createdClaimantName}`;
  ...
  if (claimantFound) {
    ...
    await page.fill(inputSelector, editedClaimantName);
    // Click on the 'Update' button
    await page.click('button:has-text("Update")');
    // Handle any dialog that appears by accepting it
    page.on('dialog', (dialog) => dialog.accept());
    // Check if the claimant name has been updated
    const updatedClaimant = await page.textContent(`text=${editedClaimantName}`);
    expect(updatedClaimant).toBe(editedClaimantName);
    // Get the success message text
    const successMessage = await page.textContent('.ant-notification-notice-description');
    // Assert that the success message is 'Claimant updated'
    expect(successMessage).toBe('Claimant updated');
  } else {
    throw new Error(`Claimant with name "${createdClaimantName}" not found.`);
  }
});

test('Should find search results', async ({ page }) => {
  const claimantToSearch = editedClaimantName;
  const searchInputSelector = '#filters_q';
  // Click the search button
  await page.click('button:has([aria-label="search"])');
  // Enter the search query
  await page.fill(searchInputSelector, claimantToSearch);
  page.locator(claimantToSearch);
  // Press 'Enter' to search
  await page.press(searchInputSelector, 'Enter');
  // Verify that the claimant is visible in the results
  const claimantExists = await page.isVisible(`text=${claimantToSearch}`);
});

test('should delete claimant successfully', async ({ page }) => {
  const claimantText = editedClaimantName;
