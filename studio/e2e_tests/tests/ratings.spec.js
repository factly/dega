...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(`${process.env.BASE_URL}ratings?sort=desc&limit=10&page=1`);
});

let createdRatingName = '';
let editedRatingName = '';
...
test.only('should create rating successfully', async ({ page }) => {
  ...
  createdRatingName = `This is a test rating ${randomString}`;
  await page.type('#creat-rating_name', createdRatingName);
...
test.only('should edit a ratings name successfully', async ({ page }) => {
  // Generate a random string
  const randomString = getRandomString(10); // Adjust the length as needed
  const editedRatingName = `This is an edited rating name ${randomString}`;
  // Click on the rating to be edited
  await page.click(`text=${createdRatingName}`);
  // Click on the 'Expand' button
  await page.click('button:has-text("Expand")');
  // Fill in the new rating name
  const inputSelector = '#creat-rating_name';
  await page.fill(inputSelector, editedRatingName);
...
test.only('should delete rating successfully', async ({ page }) => {
  const ratingText = editedRatingName;
...
test('should check for the numeric value input field up and down functionality successfully', async ({
  page,
}) => {
  // Click on the 'New Rating' button
  await page.click('button:has-text("New Rating")');
