...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(`${process.env.BASE_URL}episodes?sort=desc&limit=10&page=1`);
...
test('should create an episode successfully', async ({ page }) => {
...
test('should handle invalid URL parameters gracefully', async ({ page }) => {
  ...
  await page.goto(`${process.env.BASE_URL}episodes?sort=desc&limit=10&page=NaN`);
...
test('should display "Please enter title!" successfully, when the name input field is empty', async ({
  page,
}) => {
  test.setTimeout(90000);
  // Click on the 'Create' button
