...
import { Layout, Card, notification, BackTop, ConfigProvider, Drawer, Result} from 'antd';
  ...
  const hideSidebar =
    ...
    function checkPermissions(pathname, selected, orgs, userPermission) {
      ...
      const missingPermissions = requiredPermissions.filter(reqPerm => {
        const matchingPerm = userPermission.find(perm =>
  ...
  if (missingPermissions) {
    return (
      <Result
        status="403"
        title="403 Forbidden"
        subTitle={`You don't have required permissions: ${missingPermissions.map(
          (perm) => `${perm.resource} (${Array.isArray(perm.action) ? perm.action.join(', ') : perm.action})`
        ).join(', ')}`}
      />
    );
  }
