...
import './style.css';
const SocialCardPreview = ({ title, url, desc, image, type }) => {
  //  add time field
  const getPreviewContainer = ({ type, url, title, image, desc }) => {
    const getGoogleContainer = ({ title, url, desc }) => {
      return (
        <div style={{ flexBasis: '33%' }}>
          ...
          >
            <div className="seo-preview">
              ...
              <div className="seo-preview-link">{url}</div>
              <div className="seo-preview-title">{title}</div>
              <div className="seo-preview-desc">{desc}</div>
            </div>
          </div>
        </div>
      );
    };
    const getTwitterContainer = ({ title, url, image, desc }) => {
      return (
        <div style={{ flexBasis: '33%' }}>
          Twitter Preview:{' '}
          <div className="twitter-container">
            <div className="flex ma4" style={{ display: 'flex', margin: '0.8rem' }}>
              ...
              <div className="w-100" style={{ width: '100%' }}>
                <span className="social-og-title">{title}</span>
                ...
                <div className="social-twitter-post-preview">
                  ...
                  <div className="social-twitter-preview-content">
                    <div className="social-twitter-preview-title">{title}</div>
                    <div className="social-twitter-preview-desc">{desc}</div>
    ...
    const getFacebookContainer = ({ title, url, desc, image }) => {
      return (
        <div style={{ flexBasis: '33%' }}>
          Facebook Preview:{' '}
          <div className="og-container">
            ...
            >
              ...
              <div>
                <div className="social-og-title">{title}</div>
            ...
            <div className="social-og-preview">
              ...
              <div className="social-og-preview-bookmark">
                <div className="social-og-preview-content">
                  <div className="social-og-preview-meta">{url}</div>
                  <div className="social-og-preview-title">{title}</div>
                  <div className="social-og-preview-desc">{desc}</div>
                </div>
              </div>
            </div>
    ...
    switch (type) {
      case 'google':
        return getGoogleContainer({ url, title, desc });
      case 'twitter':
        return getTwitterContainer({ url, image, desc, title });
      case 'fb':
        return getFacebookContainer({ url, title, image, desc });
      default:
        break;
    }
  };

  return getPreviewContainer({ title, url, desc, image, type });
};
