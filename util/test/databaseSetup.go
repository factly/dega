...
func DeleteTestDB() {
	config.DB.Model(&model.Tag{}).RemoveForeignKey("space_id", "spaces(id)")
	config.DB.Model(&model.Tag{}).RemoveForeignKey("space_id", "spaces(id)")
	config.DB.Model(&model.Post{}).RemoveForeignKey("format_id", "formats(id)")
	config.DB.Model(&model.Post{}).RemoveForeignKey("featured_medium_id", "media(id)")
	config.DB.Model(&model.Space{}).RemoveForeignKey("mobile_icon_id", "media(id)")
	config.DB.Model(&model.Space{}).RemoveForeignKey("fav_icon_id", "media(id)")
	config.DB.Model(&model.Space{}).RemoveForeignKey("logo_mobile_id", "media(id)")
	config.DB.Model(&model.Space{}).RemoveForeignKey("logo_id", "media(id)")
	config.DB.Model(&model.Medium{}).RemoveForeignKey("space_id", "spaces(id)")
	config.DB.Model(&model.Format{}).RemoveForeignKey("space_id", "spaces(id)")
	config.DB.Model(&model.Category{}).RemoveForeignKey("parent_id", "categories(id)")
	config.DB.Model(&model.Category{}).RemoveForeignKey("space_id", "spaces(id)")
	config.DB.Model(&model.Category{}).RemoveForeignKey("medium_id", "media(id)")
